...
import { X, FlaskConical, BookOpen, ShieldCheck, Zap, Loader2 } from "lucide-react";
...
export default function WelcomeModal({ onTrySample, sampleLoading }: WelcomeModalProps) {
  ...
  // Auto-dismiss once loading that we triggered has finished
  useEffect(() => {
    if (!triggered) return;
    if (sampleLoading) { wasLoadingRef.current = true; return; }
    if (wasLoadingRef.current) {
      sessionStorage.setItem("docturlama-welcome-seen", "1");
      setVisible(false);
    }
  }, [triggered, sampleLoading]);
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/50 rounded-2xl max-w-lg w-full p-6 shadow-2xl">
        {triggered ? (
          <div className="flex flex-col items-center text-center py-6">
            <div className="w-14 h-14 rounded-full bg-sky-50 dark:bg-sky-900/30 flex items-center justify-center mb-4">
              <Loader2 className="w-7 h-7 text-sky-500 animate-spin" />
            </div>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Generating patient records…</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed max-w-sm">
              Claude Haiku is writing three realistic longitudinal patient records covering T2DM/CKD, CAD/HFrEF, and metabolic syndrome. Takes about 10–15 seconds.
            </p>
            <div className="flex items-center gap-2 mt-5 text-xs text-slate-400 dark:text-slate-500">
