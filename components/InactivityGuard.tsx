...
export default function InactivityGuard() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 bg-amber-950/95 border border-amber-700/60 rounded-xl px-5 py-4 shadow-2xl max-w-sm backdrop-blur-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <Clock className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-amber-200 text-sm font-medium">Session expiring</p>
        ...
        <button onClick={() => setShowWarning(false)} className="text-amber-600 hover:text-amber-400 transition-colors shrink-0">
          <X className="w-4 h-4" />
        </button>
      </div>
